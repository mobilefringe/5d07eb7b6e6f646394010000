    		...
    		<div class="site_container">
    			<div class="row top_bar_wrapper">
    				<div class="col-md-4 col-sm-6">
    				    <div id="home_hours_container" v-if="todays_hours">
    						<router-link to="hours" class="open_now">
    						    <span v-if="todays_hours.is_closed == null || !todays_hours.is_closed ">Open Today</span>
    				...
    				<div class="col-md-8 col-sm-6 hidden_phone text-right">
    					...
    					<router-link id="signup" class="caps" to="/newsletter">Sign Up</router-link>
    				</div>
    						...
    						<nav id="mobile_nav" v-show="show_mobile_menu">
    							<ul>
    								<div class="mobile_menu_site_logo">
    									<router-link to="/">
    									    <img v-if="property" src="//codecloud.cdn.speedyrails.net/sites/5e4eaf2b6e6f641683040000/image/png/1564167641000/SevenOaksBlack.png" :alt="property.name +' Mobile Logo'"/>
									    </router-link>
    								</div>
    								<li v-for="(item,key) in menu_items" class="menu_item">
    							        ...
    							        <div v-else>
    							            <b-card no-body class="mb-1">
                                                ...
                                                <b-collapse v-model="item.show_sub_menu" :id="item.name" :visible="item.show_sub_menu" accordion="my-accordion" role="tabpanel" class="accordion_body">
                                                    <b-card-body v-for="sub_menu in item.sub_menu">
                                                        <p class="card-text" @click="closeMobileMenu()">
                                                            <a v-if="sub_menu.external" :href="sub_menu.href" target="_blank">{{ sub_menu.name }}</a>
                                                            <a v-else-if="sub_menu.external_in_window" :href="sub_menu.href">{{ sub_menu.name }}</a>
                    						                <router-link v-else-if="!sub_menu.router_name" :to="sub_menu.href">{{ sub_menu.name }}</router-link>
                    						                <router-link v-else :to="{name : sub_menu.router_name, params: { new_store: sub_menu.prop }, query:{new_store: 'new_store'}} ">{{ sub_menu.name }}</router-link>
                                                        </p>
                                                    </b-card-body>
                                                </b-collapse>
                                            </b-card>
    					...
    					<nav id="primary_nav">
    						<ul>
    						    <li v-for="(item, index) in menu_items" class="menu_item" :class="{dropdown : item.sub_menu,open : item.open_dropdown}" >
    						        ...
    						        <span v-else to="#" data-toggle="dropdown" class="dropdown-toggle" aria-haspopup="true" :aria-expanded="item.open_dropdown">{{ item.name }}</span>
        ...
        return Vue.component("header-component", {
            ...
            data: function() {
                return {
                    ...
                    property_logo: "//codecloud.cdn.speedyrails.net/sites/5e4eaf2b6e6f641683040000/image/png/1564167641000/SevenOaksWhite.png",
